...
def para_project(line1, line2):
    ...
    bigli = [ [line3[0], line3[1], line2[0], line2[1]], [line3[0], line3[1], line2[2], line2[3]], [line3[2], line3[3], line2[0], line2[1]], [line3[2], line3[3], line2[2], line2[3]], line2, line3 ] 
    maxlen = 0
...
def show_desired_rect(lines, done, picture):
    ...
    for i,j,k,l in done:
        ...
        if((maxx-minx) * (maxy-miny) > max_area and tot / perimeter > perimeter_percentage):
            max_perim = tot / perimeter 
            max_area = (maxx-minx) * (maxy-miny)
            area_coords = [minx, maxx, miny, maxy]
            biggest = [i,j,k,l]
...
def get_lines(maybe):
    ...
    #image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)    #necessary if using this file manually with a non grayscale image
    ...
    lines = np.squeeze(cv2.HoughLinesP(edges, 1, np.pi/30, 20, minLineLength=25, maxLineGap=5))
